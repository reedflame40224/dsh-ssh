...
const REMOTE_RG_PATH = `$HOME/${REMOTE_HOME}/current/tools/rg`
...
class RemoteChannel {
  ...
  async rpc(method: string, params: Record<string, unknown>, timeoutMs?: number): Promise<unknown> {
    ...
    try {
      ...
      throw new RemoteRpcCallError(method, error)
    }
  }

  /** SSH-only fallback used by filesystem operations after an RPC fault. */
  async connectorCommand(command: string, timeoutMs: number): Promise<{ code: number | null; stdout: string; stderr: string }> {
    const target = targetOf(this.deps.registry, this.match.connectionId)
    if (!target) throw new Error(`连接不存在：${this.match.connectionId}`)
    try {
      ...
      throw new Error(`远端执行失败：${error instanceof Error ? error.message : String(error)}`, { cause: error })
    }
  }
  ...
  async execCommand(command: string, timeoutMs: number): Promise<{ code: number | null; stdout: string; stderr: string }> {
    ...
    if (this.runtimeUsable(record)) {
      try {
        ...
        return { code: null, stdout: '', stderr: '远端运行时返回了无效的 exec 响应' }
      } catch (error) {
        if (error instanceof RemoteRpcCallError) {
          // The remote command may have run before the response was lost. Retrying
          // it over SSH could execute a non-idempotent command twice.
          return { code: 1, stdout: '', stderr: `远端运行时 exec 调用失败：${error.message}` }
        }
        // Runtime setup failure (before an RPC was sent) is safe to fall back.
        this.warn('exec', error)
      }
    }
    try {
      return await this.connectorCommand(command, timeoutMs)
    } catch (error) {
...
class RemoteRpcCallError extends Error {
  constructor(method: string, cause: unknown) {
    super(`RPC 调用失败（${method}）：${cause instanceof Error ? cause.message : String(cause)}`, { cause })
    this.name = 'RemoteRpcCallError'
  }
}

function requireExecSuccess(
  result: { code: number | null; stdout: string; stderr: string },
  scope: string,
): { code: number | null; stdout: string; stderr: string } {
  if (result.code !== 0) {
    const detail = result.stderr.trim() || result.stdout.trim() || `exit ${String(result.code)}`
    throw new Error(`${scope} 失败：${detail}`)
  }
  return result
}
...
function infoFromStat(raw: { exists?: unknown; isDir?: unknown; isFile?: unknown; size?: unknown; mtimeMs?: unknown }): RemoteFsInfo | undefined {
  if (raw === null || typeof raw !== 'object' || typeof raw.exists !== 'boolean') {
    throw new Error('远端运行时返回了无效的 stat 响应')
  }
...
export class RemoteFs implements RemoteFsLike {
  ...
  private async fallbackStat(path: string): Promise<RemoteFsInfo | undefined> {
    ...
    const result = requireExecSuccess(await this.channel.connectorCommand(command, FS_RPC_TIMEOUT_MS), 'fs.stat')
    const line = result.stdout.trim()
    if (line === 'MISSING' || line.length === 0) return undefined
    const fields = new Map<string, string>()
    for (const field of line.split('|')) {
      const separator = field.indexOf('=')
      if (separator <= 0) throw new Error(`fs.stat 返回了无效结果：${line}`)
    ...
    const mtime = Number.parseInt(fields.get('mtime') ?? '', 10)
    if (typeRaw === undefined || !Number.isFinite(size) || !Number.isFinite(mtime)) {
      throw new Error(`fs.stat 返回了无效结果：${line}`)
    }
    const mtimeMs = mtime * 1000
    const type = typeRaw === 'directory' ? 'directory' : typeRaw === 'regular file' ? 'file' : 'other'
    return { version: `remote:${mtimeMs}:${size}`, type, size }
  }
  ...
  async readText(target: RemoteFsTarget, signal?: AbortSignal): Promise<string> {
    ...
    try {
      ...
      if (raw === null || typeof raw !== 'object' || raw.truncated === true || typeof raw.text !== 'string') {
        throw new Error(raw?.truncated === true ? '远端运行时返回了截断的文本' : '远端运行时返回了无效的文本响应')
      }
      return raw.text
    } catch (error) {
      this.channel.warn('fs.readText', error)
      const info = await this.fallbackReadableFile(path, FS_READ_CAP_BYTES)
      void info
      const result = requireExecSuccess(
        await this.channel.connectorCommand(`cat -- ${shellQuote(path)}`, FS_RPC_TIMEOUT_MS),
        'fs.readText',
      )
  ...
  private async fallbackReadableFile(path: string, maxBytes: number): Promise<RemoteFsInfo> {
    const info = await this.fallbackStat(path)
    if (info === undefined) throw new Error(`cannot read "${path}": file does not exist`)
    if (info.type !== 'file') throw new Error(`cannot read "${path}": not a regular file`)
    if (info.size !== undefined && info.size > maxBytes) {
      throw new Error(`cannot read "${path}": ${info.size} bytes exceeds the ${maxBytes}-byte limit`)
    }
    return info
  }
  ...
  async readBytes(target: RemoteFsTarget, signal: AbortSignal | undefined, maxBytes?: number): Promise<Uint8Array> {
    ...
    try {
      ...
      if (raw === null || typeof raw !== 'object' || raw.truncated === true || typeof raw.base64 !== 'string') {
        throw new Error(raw?.truncated === true ? '远端运行时返回了截断的字节流' : '远端运行时返回了无效的字节流响应')
      }
      return Uint8Array.from(Buffer.from(raw.base64, 'base64'))
    } catch (error) {
      this.channel.warn('fs.readBytes', error)
      await this.fallbackReadableFile(path, cap)
      const result = requireExecSuccess(
        await this.channel.connectorCommand(`base64 -w0 ${shellQuote(path)}`, FS_RPC_TIMEOUT_MS),
        'fs.readBytes',
      )
  ...
  async listDir(target: RemoteFsTarget, signal?: AbortSignal): Promise<RemoteFsDirEntry[]> {
    ...
    try {
      ...
      if (raw === null || typeof raw !== 'object' || !Array.isArray(raw.entries)) {
        throw new Error('远端运行时返回了无效的目录响应')
      }
  ...
  private async fallbackListDir(path: string): Promise<RemoteFsDirEntry[]> {
    ...
    const result = requireExecSuccess(await this.channel.connectorCommand(command, FS_RPC_TIMEOUT_MS), 'fs.listDir')
    const entries: RemoteFsDirEntry[] = []
  ...
  async writeText(
    ...
    try {
      ...
      if (error instanceof RemoteRpcCallError) throw error
      this.channel.warn('fs.writeText', error)
      // 回落：printf '%s' 单引号内容直写（非原子，运行时通道优先）
      requireExecSuccess(
        await this.channel.connectorCommand(`printf '%s' ${shellQuote(content)} > ${shellQuote(path)}`, FS_RPC_TIMEOUT_MS),
        'fs.writeText',
      )
    }
    const after = await this.stat(target, signal)
    if (after === undefined) throw new Error(`cannot stat written "${path}"`)
    if (after.type !== 'file') throw new Error(`cannot write "${path}": not a regular file`)
    return {
  ...
  private async readIfExists(target: RemoteFsTarget, signal?: AbortSignal): Promise<string | null> {
    ...
    if (existing.type !== 'file') throw new Error(`cannot read "${this.pathOf(target)}": not a regular file`)
  ...
  async editText(
    ...
    try {
      ...
      requireExecSuccess(
        await this.channel.connectorCommand(`printf '%s' ${shellQuote(after)} > ${shellQuote(path)}`, FS_RPC_TIMEOUT_MS),
        'fs.editText',
      )
    }
    const post = await this.stat(target, signal)
    if (post === undefined) throw new Error(`cannot stat edited "${path}"`)
    if (post.type !== 'file') throw new Error(`cannot edit "${path}": not a regular file`)
    return { version: post.version, before: contentNorm, after }
  }
}
