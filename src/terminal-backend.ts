...
export class RemoteSshPtySession implements TerminalBackendSessionLike {
  ...
  startSend(request: TerminalSendRequestLike): SshSendOperation {
    ...
    if (input.length > 0) {
      void this.terminal.write(input).catch(() => {
        // 写入失败（进程已退出）：交给轮询按 session_exit 收敛
      })
    }
    return operation
  }
  ...
  private cancelOperation(operation: SshSendOperation): void {
    ...
    // 交互中断：settle 当前 viewport（不真正给远端发 SIGINT——终端里 Ctrl-C 交互由用户侧发出）
  ...
  async close(reason: string): Promise<void> {
    ...
    try {
      ...
      throw new Error(`ssh 终端清理失败（${reason}）：${error instanceof Error ? error.message : String(error)}`)
    }
    // 收敛后在途 send 以 session_exit 终结
    this.settleActive('session_exit')
...
export function registerSshTerminalBackend(deps: SshTerminalBackendDeps): () => void {
  const idleMs = deps.idleMs ?? 800
  const timeoutMs = deps.timeoutMs ?? 30_000
  const maxReadBytes = deps.maxReadBytes ?? 64 * 1024
  const scrollbackMaxBytes = deps.scrollbackMaxBytes ?? 512 * 1024
  const scrollbackLines = 2000
  const rows = deps.rows ?? 30
